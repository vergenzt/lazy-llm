#!/usr/bin/env node
// lazy — Claude Code SessionStart activation hook
//
// Runs on every session start:
//   1. Writes flag file at $CLAUDE_CONFIG_DIR/.lazy-active (defaults to ~/.claude; statusline reads this)
//   2. Emits lazy ruleset as hidden SessionStart context
//   3. Detects missing statusline config and emits setup nudge

const fs = require('fs');
const path = require('path');
const { getDefaultMode, getClaudeDir, isShellSafe } = require('./lazy-config');
const { getLazyLLMInstructions } = require('./lazy-instructions');
const {
  clearMode,
  isCodex,
  isCopilot,
...
let output = getLazyLLMInstructions(mode);
...
if (!isCodex && !isCopilot) try {
  ...
  if (!hasStatusline && !fs.existsSync(nudgeFlagPath)) {
    ...
    if (isShellSafe(scriptPath)) {
      ...
      // tech debt: install path has shell metacharacters — don't embed it in a
