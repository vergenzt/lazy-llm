...
// tech debt: only embed the plugin install path in a statusline shell command when
...
function getClaudeDir() {
  // tech debt: CLAUDE_CONFIG_DIR overrides ~/.claude, matching Claude Code.
...
function getDefaultMode() {
  ...
  // tech debt: a default must be a runtime level (off/lite/full/ultra); review is
...
// Silence the pi "Lazy LLM loaded" startup toast while keeping lazy active.
...
function writeDefaultMode(mode) {
  // tech debt: only a runtime level can be a default; review is session-only (#377).
