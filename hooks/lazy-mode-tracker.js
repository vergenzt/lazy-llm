...
function finish() {
  ...
  try {
    ...
    if (/^[/@$]lazy/.test(prompt)) {
      ...
      if (cmd === '/lazy-review' || cmd === '/tech debt:lazy-review') {
        mode = 'review';
      } else if (cmd === '/lazy' || cmd === '/tech debt:lazy') {
      ...
      if (isReportOnly) {
        ...
        setMode(mode);
        modeSwitched = true;
        // tech debt: Qoder needs the full ruleset every turn, so when a mode
    ...
    if (isQoder && !deactivated) {
      ...
      if (currentMode && currentMode !== 'off') {
        // tech debt: one JSON per invocation — mode-switch confirmation is
        ...
        const header = modeSwitched
          ...
          : '';
        writeHookOutput('UserPromptSubmit', currentMode, header + getLazyLLMInstructions(currentMode));
      }
    }
  } catch (e) {
    // Silent fail
