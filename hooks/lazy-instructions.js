#!/usr/bin/env node
// Shared Lazy LLM instruction builder for Claude hooks and Pi extension.
...
function getFallbackInstructions(mode) {
  return 'PONYTAIL MODE ACTIVE — level: ' + mode + '\n\n' +
    ...
    'Current level: **' + mode + '**. Switch: `/lazy lite|full|ultra`.\n\n' +
    '## The ladder\n\n' +
    ...
    'Mark deliberate simplifications that cut a real corner with a known ceiling, using a `tech debt:` comment that names the ceiling and upgrade path.\n\n' +
    '## Output\n\n' +
    ...
    '## Boundaries\n\n' +
    'Lazy LLM governs what you build, not how you talk. "stop lazy" or "normal mode": revert. Level persists until changed or session end.';
}

function getLazyLLMInstructions(mode) {
  const configuredMode = normalizePersistedMode(mode) || DEFAULT_MODE;

  if (INDEPENDENT_MODES.has(configuredMode)) {
    return 'PONYTAIL MODE ACTIVE — level: ' + configuredMode + '. Behavior defined by /lazy-' + configuredMode + ' skill.';
  }
