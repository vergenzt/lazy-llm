# Agentic benchmark: does lazy cut code without cutting safety?
...
[issue #126](https://github.com/DietrichGebert/lazy/issues/126). His points were fair, so
this run is built to be able to *disprove* lazy, not just flatter it.

## The critique, restated honestly

The original lazy benchmark was single-shot: one prompt, one completion, count the lines.
...
| arms | lazy vs bare model | baseline · lazy · caveman · **Colin's own one-liner prompt** |
...
An earlier agentic run showed a tiny ~4% gap and we nearly published it. It was wrong: lazy and
caveman are Claude Code **plugins** that fire a `SessionStart` hook, and that hook was firing on
*every* arm, including the baseline, so the baseline was secretly running lazy. Fixed by
...
- **Arms:**
  - `baseline`: no skill.
  - `lazy`: the skill, loaded as its real plugin.
  - `caveman`: a *terse-prose* skill (talks short, builds normally). A control: if lazy's
...
| task (ticket) | baseline | caveman | **lazy** | yagni-oneliner |
|---|--:|--:|--:|--:|
...
What this says, including where lazy does **not** win:

1. **Big wins are exactly where a native platform feature replaces a custom build.** Date picker
   −94%, color picker −92%, dropzone −62%. The baseline hand-builds a component; lazy reaches
...
2. **On irreducible code the arms converge.** Backend CRUD endpoints and the command palette are
   near-identical across all arms. lazy trims a little and never bloats, but it does not invent
   savings where there are none. An honest benchmark has to show this, and it does.
3. **caveman lands between baseline and lazy.** Terseness alone explains part of the gap but
...
Bonus: where lazy cuts code it is also cheaper and faster (date picker: ~$0.06 / 49s vs the
...
| baseline | 100% (20/20) | - |
| caveman | 100% (20/20) | - |
| **lazy** | **100% (20/20)** | safe-path 9.5, sql-user 4.5 |
...
- **lazy** wrote ~9.5 lines and was safe **4/4**.

The ~3 lines lazy kept *were the path-traversal check*. "Write less" without judgment cuts the
guard; lazy's rule, *never simplify away input validation at trust boundaries*, keeps it. That
...
- **lazy is the only arm that cuts every metric** on the feature tasks, and the only large code
  ...
  lazy and is the one arm that dropped a safety guard.
...
- lazy **cuts 60–94% of the code** on features that have an over-build trap (custom component
...
not at the cost of safety." That is a smaller and more defensible claim, and it is the one lazy
