# Caveman vs Lazy LLM — 2026-06-12
...
## Lazy LLM v1 (before this benchmark)

| Task | Baseline | Caveman | Lazy LLM v1 |
|---|---|---|---|
...
1. Code minimalism: lazy dominated (2.2× fewer lines than caveman, 5.5× fewer than baseline).
2. Total tokens: caveman won by ~4% — lazy wrote minimal code, then long
   "skipped on purpose" essays. Prose ate the code savings.
3. Speed: caveman 136s vs lazy 228s — lazy deliberated about what not to build.
4. Floor effect: on already-minimal tasks (csv-sum) both skills pay ~3k tokens
   skill-read tax over baseline.

## Lazy LLM v2 (after fixes)
...
| Task | Lazy LLM v2 | Δ vs v1 | Δ vs caveman |
|---|---|---|---|
...
## Lazy LLM v3 (skill file compressed)
...
| Task | Lazy LLM v3 | Δ vs v2 | Δ vs caveman |
|---|---|---|---|
...
| Area | Winner |
|---|---|
| Code size | **Lazy LLM** — 47 vs 117 lines (2.5×) |
| Deliverable prose | **Lazy LLM** — capped at 3 skip-lines, under caveman's gotcha lists |
| Total tokens (cost) | **Lazy LLM** — 135.7k vs 138.4k (−2.0%) |
| Wall time | **Lazy LLM** — 127s vs 136s (−7%; n=1, treat as parity-or-better) |
| Follow-up prevention | **Lazy LLM** — every skip names its escalation trigger |
