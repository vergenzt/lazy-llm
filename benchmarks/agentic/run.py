#!/usr/bin/env python3
"""Agentic, multi-file benchmark for lazy.
...
tech debt: the claude CLI is the harness (already installed, we run inside it). No SDK
...
ARMS = {
    ...
    "lazy":       lambda: _skill("skills/lazy/SKILL.md"),
...
PLUGIN_ARMS = ("lazy", "caveman")          # arms activated via --plugin-dir (vs raw --append prompts)
PLUGIN_CACHE = Path.home() / ".claude" / "plugins" / "cache"

def _plugin_dir(name):
    """Resolve a plugin's cache dir portably -- hardcoding one machine's absolute path
    (e.g. C:\\Users\\<you>\\...) made the lazy/caveman arms unreproducible off that box.
    Order: env override -> latest version dir under ~/.claude/plugins/cache -> clear error.
    Resolved per-arm at use-site so a missing caveman install can't block a lazy-only run."""
...
# tests is still explicitly allowed, so lazy's "leave a runnable check" discipline is not suppressed.
...
def _selfcheck_split(p: Path):
    ...
    check is the runnable check lazy's rule asks for -- a positive signal, not source bloat --
...
def code_stats(workdir: Path, selfcheck_as_test: bool = False):
    ...
    reclassified from source to test, so following lazy's 'leave a runnable check' rule is not
