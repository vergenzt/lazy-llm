...
- Mark deliberate simplifications that cut a real corner with a known ceiling (global lock, O(n²) scan, naive heuristic) with a `tech debt:` comment naming the ceiling and upgrade path (`# tech debt: global lock, per-account locks if throughput matters`).
...
Lazy LLM governs what you build, not how you talk (pair with Caveman for
